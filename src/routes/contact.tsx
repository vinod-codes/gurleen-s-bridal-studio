...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Contact — Gurleen Kaur Pannu Artistry & Makeovers" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Contact — Gurleen Kaur Pannu Artistry & Makeovers" },
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          "@type": "ContactPage",
          name: "Contact — Gurleen Kaur Pannu Artistry & Makeovers",
          mainEntity: {
            "@type": "Person",
        ...
        <div className="mx-auto max-w-6xl px-6 grid md:grid-cols-2 gap-10 md:gap-16">
          <Reveal>
            ...
            <ul className="space-y-4">
              {CHANNELS.map((c) => (
                <li key={c.t} className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-t border-black/10 pt-4">
                  <p className="font-display text-xl">{c.t}</p>
                  <a href={c.href} target="_blank" rel="noreferrer" className="text-[12px] tracking-[0.24em] uppercase text-wine border-b border-gray-300/60 hover:border-black pb-1 w-fit">
                    {c.label} →
                  </a>
                </li>
              ))}
          ...
          <Reveal delay={0.15}>
            <div className="border border-black/10 bg-gray-50/40 p-8 md:p-10">
              ...
              <dl className="space-y-6 text-[16px]">
                <div>
                  ...
                  <dd className="mt-1"><a href={`tel:${PHONE}`} className="text-black hover:text-wine">{PHONE_DISPLAY}</a></dd>
                </div>
                <div>
                  <dt className="text-[11px] tracking-[0.28em] uppercase text-taupe">Studio</dt>
                  <dd className="mt-1 text-black">Mumbai, India — by appointment only</dd>
                </div>
                <div>
                  <dt className="text-[11px] tracking-[0.28em] uppercase text-taupe">Instagram</dt>
                  <dd className="mt-1 flex flex-col gap-1">
                    <a href={IG_BUSINESS} target="_blank" rel="noreferrer" className="text-black hover:text-wine">@gurleenkaurpannu_mua</a>
                ...
                <div>
                  ...
                  <dd className="mt-1 text-black">Monday – Saturday · 10:00 – 19:00 IST</dd>
                </div>
              </dl>
              <a href={WA.general} target="_blank" rel="noreferrer" className="btn-dark mt-10">Open WhatsApp</a>
            </div>
          </Reveal>
        </div>
      </section>
