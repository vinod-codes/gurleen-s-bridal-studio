...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      { title: "Bridal Makeup Price & Packages — Gurleen Kaur Pannu Artistry & Makeovers, Mumbai" },
      {
        name: "description",
        content:
          "Bridal makeup pricing and packages by Gurleen Kaur Pannu Artistry & Makeovers. Signature bridal, engagement, reception and destination wedding packages — and what shapes the final quote.",
      },
      { property: "og:title", content: "Bridal Makeup Price & Packages — Gurleen Kaur Pannu Artistry & Makeovers" },
      {
        ...
        <div className="mx-auto max-w-7xl px-6 grid md:grid-cols-3 gap-6 md:gap-8">
          {PACKAGES.map((p, i) => (
            <Reveal key={p.name} delay={i * 0.08}>
              <article className="border border-black/10 bg-gray-50/40 p-8 md:p-10 h-full flex flex-col">
                ...
                <ul className="mt-6 space-y-2 text-[15px] text-black/80">
                  {p.includes.map((it) => (
                    <li key={it} className="flex gap-3">
                      <span className="mt-2 block w-3 h-px bg-black" />
                      {it}
                    </li>
                  ))}
                </ul>
