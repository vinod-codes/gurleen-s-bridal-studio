...
export function Nav() {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-screen-2xl px-6 md:px-10 flex items-center justify-between">
        <Link to="/" className="flex flex-col leading-none">
          <span
            className={`font-display text-xl md:text-2xl tracking-widest uppercase text-black`}
          >
            GKP Artistry
          </span>
          <span
        ...
        <nav className="hidden lg:flex items-center justify-center absolute left-1/2 -translate-x-1/2 gap-10">
          {links.map((l) => (
            <Link
              key={l.label}
              to={l.to}
              className={`text-[11px] font-medium tracking-[0.15em] uppercase transition-colors text-black hover:text-gray-500`}
        ...
        <div className="hidden lg:flex items-center">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`inline-flex items-center gap-2 px-5 py-2.5 text-[11px] font-bold tracking-[0.15em] uppercase border transition-colors border-black text-black hover:bg-black hover:text-white`}
          >
            WhatsApp
          </a>
        </div>
        ...
        <button
          aria-label="Menu"
          onClick={() => setOpen((v) => !v)}
          className={`lg:hidden flex flex-col gap-1.5 text-black`}
        >
      ...
      {open && (
        <div className="lg:hidden px-6 py-8 flex flex-col gap-5 bg-white border-t border-black/10 absolute top-full left-0 right-0 shadow-lg">
          {links.map((l) => (
            <Link
              key={l.label}
              to={l.to}
